    ...
    function betsJson()
    {
        ...
        if(empty($type)){
            if($user['is_robot']==1){
                $list = Db::name('robot_bets')->alias("a")
                    ->join("lz_lotteries b","a.sequencenum=b.sequencenum and a.type=b.type","left")
                    ...
                    ->field("a.*,b.outcome,b.type as btype")
                    ->order('a.id desc')
                    ->paginate(10000);
            ...
            if($user['is_robot']==1) {
                $list = Db::name('robot_bets')->alias("a")
                    ->join("lz_lotteries b", "a.sequencenum=b.sequencenum and a.type=b.type", "left")
                    ->where(['a.userid' => $this->user['userid'], 'a.section' => 0, 'a.type' => $type])
                    ->whereTime('a.addtime', 'between', ["$analyze_firstday 00:00:00", "$analyze_firstday 23:59:59"])
                    ->field("a.*,b.outcome,b.type as btype")
                    ->order('a.id desc')
                    ->paginate(12);
        ...
        if(!empty($type)) {
            ...
            foreach($list['data'] as $v) {
                if($v['operation'] == 1 && $v['btype'] != $type) {
                    
                } else if($v['btype'] != $type) {
                    $v['outcome'] = '';
                    $listData[] = $v;
                } else {
                    $listData[] = $v;
...
/* 客服聊天 */
    ...
    function logout(){
        session(null);
        echo '<script>window.location.href="https://v8.yhdsqq03.top/index/index/login2";</script>';
    }
}