...
class Banner extends Init
{
    ...
    function index()
    {
        ...
        $substation = Db::name('banner')->order("order_index asc")->paginate($page_size);
    ...
    public function check()
    {
        ...
        if (isset($list['id']) && isset($list['check'])) {
            ...
            if (!is_null($id)) {
                ...
                Db::name('banner')->where('id', $id)->update(['status' => $isshow]);
    ...
    function edit()
    {
        ...
        if (request()->isPost()) {
            ...
            if(!empty($params['imgurl'])){
                $data['imgurl'] = $params['imgurl'];
            }
            $data['order_index'] = $params['order_index'];
            $data['url'] = $params['url'];
            $data['is_go'] = $params['is_go'];
    ...
    function add()
    {
        if (request()->isPost()) {
            ...
            $data['is_go'] = $params['is_go'];
            $data['status'] = $params['status'];
            $Substation = Db::name('banner')->where(['name' => $data['name']])->find();
            if (!empty($Substation)) {
                $this->error("您已添加此banner！");
            }
            $result = Db::name('banner')->insertGetId($data);
            if ($result > 0) {
                $this->success("添加成功");
            } else {
                $this->error("添加失败");
    ...
    function del()
    {
        $result = Db::name('banner')->where('id', input('post.id'))->delete();
        if ($result) {
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
    ...
    function batches_delete()
    {
        ...
        $result = Db::name('banner')->where('id', 'in', $ids)->delete();
        if ($result) {
            $this->success('批量删除成功');
        } else {
            $this->error('删除失败');
