        ...
        <div class="layui-tab-item layui-show">
           <div style="margin-bottom: 10px">
                <ul class="layui-tab-title main-tab-title" style="display: flex;justify-content: space-between;padding-right: 0;padding-left: 0">
                    <li>
                        <div class="main-tab-item">添加白名单</div>
                    </li>
                </ul>
            </div>
            <form class="layui-form" action="" method="post" enctype="multipart/form-data" style="width: 95%">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="ip" required lay-verify="title" placeholder="请输入IP" autocomplete="off" class="layui-input">
                    </div>
                </div>
                ...
                <div class="layui-form-item layui-form-text">
                    ...
                    <div class="layui-input-block" style="width: 74.4%">
                        <input type="radio" name="status" lay-filter="tested" value="0" title="关闭" checked>
                        <input type="radio" name="status" lay-filter="tested" value="1" title="开启" >
                    </div>
                </div>
...
<script>
    function backindex(data){
        if(data.code==1){
            location.href="{:url('whitelist/add')}";
        }
    }
    layui.use(['form', 'upload','laydate'], function () {
       ...
       form.on('submit(fsubmit)', function(data){  
             parent.$.fn.jcb.post('{:url("whitelist/add")}',data.field,false,backindex);
             return false; 
        }); 
