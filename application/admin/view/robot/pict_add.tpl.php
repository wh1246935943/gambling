                    ...
                    <tr>
                        ...
                        <td style="text-align: center;position: relative">
                            <div class="site-demo-upload">
                                <img id="avatar_img{$arr}" src="" style="width: 150px;height: 150px;">
                                <div class="site-demo-upbar" style="display: inline-block">
                                    <input type="file" name="avatar1" lay-ext="jpg|jpeg|png|gif" class="layui-upload-file" id="test{$arr}">
                                    <input type="hidden" name="robot_arr[{$arr}][headimgurl]" id="thumb{$arr}" value="">
                                </div>
                            </div>
                        </td>
...
<script>
    function backindex(data){
        if(data.code==1){
            location.href="{:url('robot/pict_add')}";
        }
    } 
    layui.use(['form', 'upload','laydate'], function () {
       ...
       form.on('submit(fsubmit)', function(data){  
             parent.$.fn.jcb.post('{:url("robot/pict_add")}',data.field,false,backindex);
             return false; 
        });
        layui.upload({
        ...
        layui.upload({
            url: '/admin/yuming/upload'
            , elem: '#test5' //指定原始元素，默认直接查找class="layui-upload-file"
            , method: 'post' //上传接口的http类型
            , success: function (res) {
                if (res.code == 0) {
                    $("#avatar_img5").attr('src',res.src);
                    $("#thumb5").val(res.src);
                } else {
                    layer.alert(res.msg);
                }
        ...
        layui.upload({
            url: '/admin/yuming/upload'
            , elem: '#test6' //指定原始元素，默认直接查找class="layui-upload-file"
            , method: 'post' //上传接口的http类型
            , success: function (res) {
                if (res.code == 0) {
                    $("#avatar_img6").attr('src',res.src);
                    $("#thumb6").val(res.src);
                } else {
                    layer.alert(res.msg);
                }
        ...
        layui.upload({
            url: '/admin/yuming/upload'
            , elem: '#test7' //指定原始元素，默认直接查找class="layui-upload-file"
            , method: 'post' //上传接口的http类型
            , success: function (res) {
                if (res.code == 0) {
                    $("#avatar_img7").attr('src',res.src);
                    $("#thumb7").val(res.src);
                } else {
                    layer.alert(res.msg);
                }
        ...
        layui.upload({
            url: '/admin/yuming/upload'
            , elem: '#test8' //指定原始元素，默认直接查找class="layui-upload-file"
            , method: 'post' //上传接口的http类型
            , success: function (res) {
                if (res.code == 0) {
                    $("#avatar_img8").attr('src',res.src);
                    $("#thumb8").val(res.src);
                } else {
                    layer.alert(res.msg);
                }
        ...
        layui.upload({
            url: '/admin/yuming/upload'
            , elem: '#test9' //指定原始元素，默认直接查找class="layui-upload-file"
            , method: 'post' //上传接口的http类型
            , success: function (res) {
                if (res.code == 0) {
                    $("#avatar_img9").attr('src',res.src);
                    $("#thumb9").val(res.src);
                } else {
                    layer.alert(res.msg);
                }
